...
| C2 | 투자성향 ↔ **펀드 위험등급 Eligibility Mapping**: 가입 불가능 펀드는 Reasoning 전 Candidate Space에서 제거하고 Post-Reasoning에서 재검증 | **Human-approved Business Fact (HD-2, 매핑 HD-2.1 2026-09-01)** | **SRC-096** (Human-confirmed Official; 원문 확보 시 교체): 안정형 6 / 안정추구형 5~6 / 위험중립형 4~6 / 적극투자형 3~6 / 공격투자형 1~6 | **구현 (REV-001)**: Pre-Reasoning — 권유 가능/불가 등급을 Constraint Section으로 전달 · Post — 불가 등급 라벨이 next_actions[].action에 있으면 FAIL, condition/brief면 REVIEW(부정문·기존 보유 가능). 기존 보유 상품은 검사하지 않음 | RUN_002에서 관찰 | GC-02, 05, 06, 17 (P0: GC-06) |
...
- C1 Validator는 모델이 자기 기재한 `risk_level` 라벨만 검사한다. direction 내용과 라벨의 정합성은 Evaluator가 별도 확인 (CASE_001·GC-01: 정합). C2·C3는 텍스트 스캔(부정문 미구분 → REVIEW 등급 사용). REV-001부터 next_actions[] 기준.
